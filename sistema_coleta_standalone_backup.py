"""
Sistema de Coleta de Imagens para FuncionÃ¡rios - Standalone
AplicaÃ§Ã£o offline para coletar fotos de produtos com anotaÃ§Ãµes
"""
...
class SistemaColetaImagens:
    ...
    def init_database(self):
        ...
        # Tabela de produtos (serÃ¡ preenchida manualmente ou importada)
        cursor.execute('''
        ...
        # Tabela de anotaÃ§Ãµes (bounding boxes)
        cursor.execute('''
    ...
    def criar_interface(self):
        ...
        # Coluna 3 - Controles e anotaÃ§Ãµes
    ...
    def criar_painel_produtos(self, parent):
        ...
        tk.Button(
            frame,
            text="âž• Adicionar Novo Produto",
    ...
    def criar_painel_imagem(self, parent):
        ...
        self.label_instrucoes = tk.Label(
            frame,
            text="ðŸ“– Selecione um produto e clique + arraste na imagem para desenhar o box",
    ...
    def criar_painel_controles(self, parent):
        """Painel direito com controles e lista de anotaÃ§Ãµes"""
        frame = tk.Frame(parent, bg='white', relief='ridge', borderwidth=2)
        ...
        tk.Label(
            frame,
            text="ðŸ’¡ Dica: DÃª duplo clique para remover",
        ...
        tk.Button(
            frame_acoes,
            text="ðŸ“¤ Exportar para SincronizaÃ§Ã£o",
    ...
    def on_produto_selecionado(self, event):
        """Callback quando um produto Ã© selecionado"""
    ...
    def adicionar_produto(self):
        ...
        tk.Label(
            dialog,
            text="DescriÃ§Ã£o Completa do Produto:",
            bg='white',
            font=('Segoe UI', 10, 'bold')
        ...
        def salvar():
            ...
            if not descricao:
                messagebox.showwarning("Aviso", "Digite a descriÃ§Ã£o do produto!")
                return
    ...
    def tirar_foto_webcam(self):
        ...
        try:
            ...
            if not cap.isOpened():
                messagebox.showerror("Erro", "NÃ£o foi possÃ­vel acessar a webcam!")
                return
                
            messagebox.showinfo("Webcam", "Pressione ESPAÃ‡O para capturar ou ESC para cancelar")
            
            while True:
                ret, frame = cap.read()
                if ret:
                    cv2.imshow('Webcam - Pressione ESPAÃ‡O para capturar', frame)
            ...
            messagebox.showerror("Erro", "OpenCV nÃ£o instalado!\nInstale com: pip install opencv-python")
    ...
    def on_canvas_click(self, event):
        """InÃ­cio do desenho do bounding box"""
    ...
    def atualizar_lista_anotacoes(self):
        """Atualiza a lista de anotaÃ§Ãµes e contador"""
    ...
    def remover_anotacao_selecionada(self, event):
        """Remove uma anotaÃ§Ã£o da lista"""
    ...
    def limpar_tudo(self):
        """Limpa todas as anotaÃ§Ãµes"""
        if self.bboxes and messagebox.askyesno("Confirmar", "Deseja limpar todas as anotaÃ§Ãµes?"):
    ...
    def salvar_anotacoes(self):
        """Salva as anotaÃ§Ãµes no banco de dados local"""
        if not self.imagem_path:
            messagebox.showwarning("Aviso", "Carregue uma imagem primeiro!")
            return
            
        if not self.bboxes:
            messagebox.showwarning("Aviso", "Adicione pelo menos uma anotaÃ§Ã£o!")
            return
            
        # Copiar imagem para diretÃ³rio de dados
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        messagebox.showinfo(
            "Sucesso",
            f"âœ“ Imagem salva com {len(self.bboxes)} anotaÃ§Ãµes!\n\n"
            f"Arquivo: {novo_nome}"
        )
        
        # Limpar para prÃ³xima
        self.limpar_tudo()
        self.canvas.delete('all')
        self.imagem_path = None
    ...
    def exportar_dados(self):
        """Exporta dados para sincronizaÃ§Ã£o com o servidor"""
        ...
        # Exportar imagens nÃ£o sincronizadas
        ...
        messagebox.showinfo(
            ...
            f"âœ“ {len(imagens)} imagens exportadas!\n\n"
            f"Pasta: {export_dir}\n\n"
