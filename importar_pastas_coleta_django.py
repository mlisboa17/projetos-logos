...
Cria registros em ImagemProdutoPendente (sistema de coleta Django)
"""
...
def importar_pasta_coleta():
    ...
    for base_dir in pastas_busca:
        ...
        for pasta_produto in base_dir.iterdir():
            ...
            if nome_produto.startswith('.') or nome_produto in ['anotacoes', 'treino', 'teste']:
                continue
    ...
    print()
    resposta = input("Deseja importar essas imagens para o Django? (s/n): ")
    ...
    lote = LoteFotos.objects.create(
        usuario=usuario,
        descricao=f"Importação de pastas de coleta - {datetime.now().strftime('%d/%m/%Y %H:%M')}",
    ...
    # Atualizar lote
    lote.data_finalizacao = datetime.now()
    lote.save()
